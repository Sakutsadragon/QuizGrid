...
module.exports.register = async (req, res, next) => {
  try {
    ...
    const usernameCheck = await QuizUser.findOne({ username }).catch((err) => {
    ...
    const emailCheck = await QuizUser.findOne({ email }).catch((err) => {
    ...
    // Return user data (client will store it in localStorage)
    return res.status(201).json({ status: true, user: user.toJSON() });
  } catch (ex) {
    console.error("Registration error:", ex);
    return res.status(500).json({ msg: ex.message || "Internal Server Error", status: false });
  }
};
...
module.exports.login = async (req, res, next) => {
  try {
    ...
    const user = await QuizUser.findOne({ username }).catch((err) => {
    ...
    // Return user data (client will store it in localStorage)
    return res.status(200).json({ status: true, user: user.toJSON() });
  } catch (ex) {
