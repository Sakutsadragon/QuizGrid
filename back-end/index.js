...
io.on('connection', (socket) => {
  ...
  socket.on('joinRoom', ({ roomId, userId, username }) => {
    ...
    socket.join(roomId);
    rooms[roomId].players.set(userId, { socketId: socket.id, username, score: 0 });
    ...
    if (playersArray.length === 2 && !rooms[roomId].gameActive) {
      ...
      io.to(roomId).emit('gameStarted', {
        players: playersArray.map(([id, data]) => ({ userId: id, username: data.username, score: data.score })),
        currentTurn: hostId,
  ...
  socket.on('cellClick', ({ roomId, index }) => {
    ...
    const playerId = Array.from(room.players.entries()).find(([id, p]) => p.socketId === socket.id)?.[0];
  ...
  socket.on('submitAnswer', ({ roomId, cellIndex, isCorrect, playerId, nextTurn }) => {
    ...
    if (isCorrect) {
      ...
      const opponentId = Array.from(room.players.keys()).find(id => id !== playerId);
      if (opponentId) room.players.get(opponentId).score += 1;
    }

    io.to(roomId).emit('updateOwnership', { cellIndex, owner });
    io.to(roomId).emit('updatePlayers', {
      players: Array.from(room.players.entries()).map(([id, data]) => ({ userId: id, username: data.username, score: data.score })),
  ...
  const checkWinCondition = (room, roomId) => {
    ...
    for (const pattern of patterns) {
      if (pattern.every(i => room.cellOwnership[i] === 'P') || pattern.every(i => room.cellOwnership[i] === 'C')) {
        ...
        const finalScores = players.map(([id, data]) => ({ userId: id, username: data.username, score: data.score }));
        const message = `Game over in room ${roomId}. Winner: ${room.players.get(winnerId).username}`;
        io.to(roomId).emit('gameOver', { winner: winnerId, finalScores, message });
        return;
      }
    }
    ...
    if (room.usedCells.size === 25) {
      room.gameActive = false;
      const scores = Array.from(room.players.entries()).map(([id, data]) => ({ userId: id, username: data.username, score: data.score }));
      ...
      io.to(roomId).emit('gameOver', { winner: winner.userId, finalScores: scores, message });
    }
  };
  ...
  socket.on('restartGame', (roomId) => {
    ...
    if (room && !room.gameActive) {
      ...
      io.to(roomId).emit('gameStarted', { players: [], currentTurn: null, cellOwnership: room.cellOwnership });
    }
  });
  ...
  socket.on('disconnect', () => {
    for (let roomId in rooms) {
      const room = rooms[roomId];
      const userEntry = Array.from(room.players.entries()).find(([_, data]) => data.socketId === socket.id);
      if (userEntry) {
        const [userId, userData] = userEntry;
        room.players.delete(userId);
        io.to(roomId).emit('playerLeft', { userId, username: userData.username });
        if (room.players.size < 2 && room.gameActive) {
          room.gameActive = false;
          io.to(roomId).emit('gameEnded', { message: 'Game ended due to player disconnection' });
        }
      }
    }
  });
